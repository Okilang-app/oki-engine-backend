"""Integration tests for SHA-256 deduplication on upload completion."""
    ...
    async with session_factory.begin() as session:
        await session.execute(
            text(
                ...
                "values (:id, :slug, 'Deduplication tenant')"
            ),
            {"id": organization_id, "slug": f"dedup-{organization_id}"},
        )
        await session.execute(
            text(
            ...
            {
                "id": user_id,
                "subject": f"dedup-{user_id}",
                "email": f"dedup-{user_id}@example.test",
            },
        )
    try:
...
def creator_payload(organization_id: UUID) -> dict[str, object]:
    return {
        ...
        "legal_name": "Dedup Creator LLC",
        "display_name": "Dedup Creator",
        "primary_email": "creator@example.test",
        "channels": [],
    }


async def test_deduplication_reuses_existing_storage_key(
    ...
    # Create first creator and asset
    creator1_resp = await client.post("/api/creators", json=creator_payload(organization_id))
    assert creator1_resp.status_code == 201
    creator1 = creator1_resp.json()

    asset1_resp = await client.post(
        "/api/assets",
        json={"creator_id": creator1["id"], "title": "Original"},
    )
    assert asset1_resp.status_code == 201
    asset1 = asset1_resp.json()
    ...
    first_storage_key = complete1.json()["storage_key"]
    assert first_storage_key is not None

    # Create second asset with same hash
    creator2_resp = await client.post("/api/creators", json=creator_payload(organization_id))
    assert creator2_resp.status_code == 201
    creator2 = creator2_resp.json()

    asset2_resp = await client.post(
        "/api/assets",
        json={"creator_id": creator2["id"], "title": "Duplicate"},
    )
    assert asset2_resp.status_code == 201
    asset2 = asset2_resp.json()
    ...
    second_storage_key = complete2.json()["storage_key"]

    # The duplicate should reuse the original storage key
